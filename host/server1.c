...
static void probe_device(libusb_device_handle *handle)
{
    ...
    for (int i = 0; i < config->bNumInterfaces; i++) {
        ...
        for (int alt = 0; alt < iface->num_altsetting; alt++) {
            ...
            printf("  Interface:\n    Number:             %d\n    Alt Setting:        %d\n    Class/Subclass:     0x%02x / 0x%02x\n    Protocol:           0x%02x\n    Endpoints:          %d\n",
                   desc_if->bInterfaceNumber, desc_if->bAlternateSetting,
                   desc_if->bInterfaceClass, desc_if->bInterfaceSubClass, desc_if->bInterfaceProtocol,
                   desc_if->bNumEndpoints);
            ...
            for (int e = 0; e < desc_if->bNumEndpoints; e++) {
                ...
                // Detect CDC interface (class 0x02) and vendor interface (class 0xFF)
...
int main(void)
{
    ...
    if (libusb_claim_interface(devh, dev_info.cdc_interface) < 0 ||
        libusb_claim_interface(devh, dev_info.vendor_interface) < 0) {
